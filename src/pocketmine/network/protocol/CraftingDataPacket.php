	...
	private static function writeShapelessRecipe(ShapelessRecipe $recipe, BinaryStream $stream, $playerProtocol){
		...
		foreach($recipe->getIngredientList() as $item){	
			if ($playerProtocol >= Info::PROTOCOL_360) {
				...
				if($playerProtocol >= Info::PROTOCOL_418) $stream->putSlotWithoutStackId($item, $playerProtocol);
				else $stream->putSlot($item, $playerProtocol);
			}
		}

		$stream->putVarInt(1);
		if($playerProtocol >= Info::PROTOCOL_418) $stream->putSlotWithoutStackId($recipe->getResult(), $playerProtocol);
		else $stream->putSlot($recipe->getResult(), $playerProtocol);
		...
		for($z = 0; $z < $recipe->getWidth(); ++$z){
			for($x = 0; $x < $recipe->getHeight(); ++$x){
				...
				if ($playerProtocol >= Info::PROTOCOL_360) {
		            ...
		            if($playerProtocol >= Info::PROTOCOL_418) $stream->putSlotWithoutStackId($slot, $playerProtocol);
	            	else $stream->putSlot($slot, $playerProtocol);
				}
			}
		}
