...
class QueryHandler{
	...
	public function __construct(){
		...
		$this->server->getLogger()->info("Запущена прослушка статуса GS4");
		$addr = ($ip = $this->server->getIp()) != "" ? $ip : "0.0.0.0";
		$port = $this->server->getPort();
		$this->server->getLogger()->info("Query порт установлен на: $port");
		/*
		...
		*/
		...
		$this->regenerateInfo();
		$this->server->getLogger()->info("Query стартовал на адресе: {$addr}, порту: {$port}.");
	}
