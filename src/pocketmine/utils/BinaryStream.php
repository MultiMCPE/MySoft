...
use pocketmine\nbt\NBT;
use pocketmine\nbt\{tag\IntTag, tag\Compound};
use pocketmine\utils\MetadataConvertor;
use pocketmine\network\protocol\Info;
use pocketmine\network\protocol\PEPacket;
use pocketmine\{Player, entity\Entity};
...
class BinaryStream {
	...
	public function writeMetadata(array $data, $playerProtocol){
	...
	public function get($len) {
		...
		if($remaining < $len){
			throw new StupidException("Не хватает байтов в буфере: нужно: {$len}, а тут {$remaining}");
		}
		if (strlen($this->buffer) < $this->offset + $len) {
			throw new StupidException('Ошибка получения nbt');
		}
	...
	public function getSlotNew($playerProtocol, $withStackId = true) {
	...
	public function putSlotNew(Item $item, $playerProtocol, $withStackId = true) {
		if ($item->getId() === 0) {
	...
	public function getSlot($playerProtocol, $withStackId = true) {
	    if($playerProtocol >= Info::PROTOCOL_431) return $this->getSlotNew($playerProtocol, $withStackId);
		$id = $this->getSignedVarInt();
		if ($id == 0) {
			return Item::get(Item::AIR, 0, 0);
		}
	...
	public function putSlot(Item $item, $playerProtocol, $withStackId = true) {
	    if($playerProtocol >= Info::PROTOCOL_431) return $this->putSlotNew($item, $playerProtocol, $withStackId);
		if ($item->getId() === 0) {
			$this->putSignedVarInt(0);
			return;
		}
	...
	public function putSerializedSkin($playerProtocol, $skinId, $skinData, $skinGeometryName, $skinGeometryData, $capeData, $additionalSkinData) {
		...
		if ($playerProtocol >= Info::PROTOCOL_425) {
		...
		if($playerProtocol < Info::PROTOCOL_459){
