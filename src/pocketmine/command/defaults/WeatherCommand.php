	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($sender instanceof Player){
			...
			if($wea >= 0 and $wea <= 3){
				...
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.weather.changed", [$sender->getLevel()->getFolderName()]));
				return true;
				/*if(WeatherManager::isRegistered($sender->getLevel())){
					...
					$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.weather.noregistered", [$sender->getLevel()->getFolderName()]));
					return false;
				}*/
			}else{
		...
		if($wea >= 0 and $wea <= 3){
			...
			$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.weather.changed", [$level->getFolderName()]));
			return true;
			/*if(WeatherManager::isRegistered($level)){
				...
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.weather.noregistered", [$level->getFolderName()]));
				return false;
			}*/
		}else{
			$sender->sendMessage(TextFormat::RED . "%pocketmine.command.weather.invalid");
