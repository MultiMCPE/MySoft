		...
		if($player instanceof Player){
			...
			if(strcasecmp(substr($args[0], -1), "L") == 0){
				...
				if($level > 0){
					...
					$sender->sendMessage($sender->getServer()->getLanguage()->translateString("%commands.xp.success.levels", [$level, $name]));
					$player->getLevel()->broadcastLevelSoundEvent($player, LevelSoundEventPacket::SOUND_LEVELUP);
					return true;
				}elseif($level < 0){
					$player->takeXpLevel((int) -$level);
					$sender->sendMessage($sender->getServer()->getLanguage()->translateString("%commands.xp.success.negative.levels", [-$level, $name]));
					return true;
				}
			}else{
				if(($xp = (int) $args[0]) > 0){ //Set Experience
					...
					$sender->sendMessage($sender->getServer()->getLanguage()->translateString("%commands.xp.success", [$name, $args[0]]));
					return true;
				}elseif($xp < 0){
					$sender->sendMessage($sender->getServer()->getLanguage()->translateString("%commands.xp.failure.withdrawXp"));
					return true;
				}
			}
