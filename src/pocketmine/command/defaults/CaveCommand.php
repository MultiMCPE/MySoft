...
class CaveCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.commands.cave.info", [$caves[0], $caves[1], $caves[2], $caves[4]]));
		$sender->sendMessage($sender->getServer()->getLanguage()->translateString(TextFormat::YELLOW . "%pocketmine.commands.cave.start"));
		$sender->sendMessage($pos->x . " " . $pos->y . " " . $pos->z);
		$this->caves($pos, $caves);
		$sender->sendMessage($sender->getServer()->getLanguage()->translateString(TextFormat::GREEN . "%pocketmine.commands.cave.success"));
		return true;
	}
