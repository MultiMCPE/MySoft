...
class GamemodeCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if($target->setGamemode($gameMode) == false){
			...
			if($target === $sender){
				Command::broadcastCommandMessage($sender, $sender->getServer()->getLanguage()->translateString("commands.gamemode.success.self", [' ', ' ', Server::getGamemodeString($gameMode)]));
			}else{
				$target->sendMessage($sender->getServer()->getLanguage()->translateString("gameMode.changed"));
				Command::broadcastCommandMessage($sender, $sender->getServer()->getLanguage()->translateString("commands.gamemode.success.other", [$target->getName(), Server::getGamemodeString($gameMode)]));
			}
		}
		return true;
	}
