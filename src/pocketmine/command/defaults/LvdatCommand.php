...
class LvdatCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if(!$this->autoLoad($sender, $levname)){
			$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.lvdat.nofound", [$levname]));
			return false;
		}
		...
		switch($o){
			case "fixname":
				...
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.lvdat.fixname", [$level->getFolderName()]));
				break;
			case "help":
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("commands.generic.usage", [$this->usageMessage]));
			...
			case "seed":
				...
				$provider->setSeed($p);
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("pocketmine.command.lvdat.changed", [$level->getFolderName(), $o]));
				break;
			case "name":
				if($p == ""){
					$sender->sendMessage("%commands.generic.opt.missing");
