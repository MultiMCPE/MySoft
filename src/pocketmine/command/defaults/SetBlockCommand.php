		...
		if($itemblock instanceof ItemBlock){
			...
			if($pos instanceof Vector3){
				...
				if($level->setBlock($pos, $block)){
					...
					$sender->sendMessage(TextFormat::RED . $sender->getServer()->getLanguage()->translateString("commands.generic.exception", []));
					return false;
				}
			}
		}else{
			$sender->sendMessage(TextFormat::RED . $sender->getServer()->getLanguage()->translateString("command.setblock.invalidBlock", []));
			return false;
		}
		return true;
	}
