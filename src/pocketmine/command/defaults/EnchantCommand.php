...
class EnchantCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($enchantment->getId() === Enchantment::TYPE_INVALID){
			...
			if($enchantment->getId() === Enchantment::TYPE_INVALID){
				$sender->sendMessage($sender->getServer()->getLanguage()->translateString("commands.enchant.notFound", [$enchantment->getId()]));
				return true;
			}
		}
		...
		if($enchantLevel > $maxLevel or $enchantLevel <= 0){
			$sender->sendMessage($sender->getServer()->getLanguage()->translateString("commands.enchant.maxLevel", [$maxLevel]));
			return true;
		}
		...
		if($item->getId() <= 0){
			$sender->sendMessage($sender->getServer()->getLanguage()->translateString("commands.enchant.noItem"));
			return true;
		}

		if(Enchantment::getEnchantAbility($item) === 0){
			$sender->sendMessage($sender->getServer()->getLanguage()->translateString(TextFormat::RED . "%commands.enchant.cantEnchant"));
			return true;
		}
		...
		self::broadcastCommandMessage($sender, $sender->getServer()->getLanguage()->translateString("%commands.enchant.success"));
		return true;
	}
}