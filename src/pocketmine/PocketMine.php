...
namespace pocketmine {
	...
	const VERSION = '1.2.0';
	const API_VERSION = "X";
	const CODENAME = "MySoft";
	const MINECRAFT_VERSION = "1.x";
	const MINECRAFT_VERSION_NETWORK = "1.x";
  ...
  error_reporting(E_ALL);
	...
	$logger = new MainLogger(\pocketmine\DATA . "server.log", true); //, \pocketmine\ANSI);
    $logger->registerStatic();
	if(!ini_get("date.timezone")){
		if(($timezone = detect_system_timezone()) and date_default_timezone_set("Europe/Moscow")){
			...
			if($response = Utils::getURL("http://ip-api.com/json")
				...
				and date_default_timezone_set("Europe/Moscow")
			){
	...
	if(\Phar::running(true) === ""){
		$logger->warning("Non-packaged MySoft installation detected, do not use on production.");
	}
